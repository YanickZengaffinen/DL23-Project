...
class Uncertainty(ModelWrapper):
    def init_model(self, dataset_name: str, ways: int, shots: int):
        ...
        if dataset_name == "Omniglot":
            ...
            self._model.load_state_dict(torch.load(f'uncertainty/models/omniglot/best-binary-classifiers-{TASK_EMBEDDING_SIZE_OMNIGLOT}.pt', map_location=self.device))
            ...
            self._model.load_state_dict(torch.load(f'uncertainty/models/mini-imagenet/best-binary-classifiers-{TASK_EMBEDDING_SIZE_MINIIMAGENET}.pt', map_location=self.device))
    ...
    def reset_model(self):
        if self.dataset_name == 'Omniglot':
            prepare_model_for_adaptation(self._model, f'uncertainty/models/omniglot/best-meta-embedding-maml-{TASK_EMBEDDING_SIZE_OMNIGLOT}.pt', self.ways, device=self.device)
        elif self.dataset_name == 'MiniImageNet':
            prepare_model_for_adaptation(self._model, f'uncertainty/models/mini-imagenet/best-meta-embedding-maml-{TASK_EMBEDDING_SIZE_MINIIMAGENET}.pt', self.ways, device=self.device)
